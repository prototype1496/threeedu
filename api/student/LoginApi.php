...
class LoginApi {
    ...
    public function authenticate() {
        $validUser = $this->verifyUser();
        if ($validUser) {
            $validPassword = $this->verifyPassword($validUser, $this->user['password']);
            if ($validPassword) {
                return true;
            } else {
                $validUser['error'] = true;
            ...
            $response['status_code_header'] = 'HTTP/1.1 401 Unauthorized';
            header($response['status_code_header']);
            $response['body'] = "username invalid";
            echo json_encode($response);
        }
    }
    ...
    private function verifyUser() {
        ...
        $stm->execute(array(':username' => $this->userId));
        if ($stm->rowCount() > 0) {
    ...
    function verifyPassword($user_data, $entered_password) {
        $hushed_password = $user_data['Password'];
        return password_verify($entered_password, $hushed_password);;
    }
