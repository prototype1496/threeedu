...
if (isset($_POST['btn_reg_pupil'])) {
    ...
    $assessmentTypeId = isset($_POST['accesmenttype_id']) ? $_POST['accesmenttype_id'] : "accesmenttype_id";
    $assessmentName = isset($_POST['assessment_name']) ? $_POST['assessment_name'] : "assessment_name";
    $count = 0;
    $data = array();
    ...
    foreach ($studentPublicId as $key => $value) {
        if ($count < $sizeOfIdArray) {
            ...
            array_push($data, array($studentPublicId[$count], $assessmentTypeId, $teacherName, null, $teacherComment, null, "HeadTeacher", $assessmentName, $UpdatedBy));
            $count++;
        }
    }
    if (count($data) > 0) {
        if (SuperModel::addTeacherComments($data)) {
            echo "<script>               
            $(document).ready(
            function(){                
               $.jnoty('Comments Submitted Successfully', {
            sticky: false,
            header: 'Success',
            theme: 'jnoty-success',
    ...
    foreach ($studentPublicId as $key => $value) {
        if ($count < $sizeOfIdArray) {
            ...
            array_push($data, array("studentPublicId" => $studentPublicId[$count], "assessmentTypeId" => $assessmentTypeId, "teacherName" => $teacherName, "commentData" => $teacherComment, "status" => "Done", "assessmentName" => $assessmentName, "updatedBy" => $UpdatedBy));
            $count++;
        }
    }
    if (count($data) > 0) {
        if (SuperModel::addHeadTeacherComments($data)) {
            echo "<script>               
            $(document).ready(
            function(){                
               $.jnoty('Comments Submitted Successfully', {
    ...
    if (SuperModel::saveTeacherSignInTime($clickedBtn,$_POST['publicId'],$_POST['tenantId'])) {
        echo "<script>               
            $(document).ready(
            function(){                
               $.jnoty('$clickedBtn Details Submitted Successfully', {
            sticky: false,
            header: 'Success',
            theme: 'jnoty-success',
            close: function() {window.location.replace('/threeedu/view/teacher/loginTime.php')},
            });   
            }); 
            </script>";
    } else {
        echo "<script>               
            $(document).ready(
            function(){                
               $.jnoty('$clickedBtn time already saved', {
            sticky: false,
            header: 'Warning',
            theme: 'jnoty-warning',
            close: function() {window.location.replace('/threeedu/view/teacher/loginTime.php')},
