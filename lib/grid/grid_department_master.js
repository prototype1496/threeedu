$(document).ready(function () {
    ...
    $(".add-new").click(function () {
        ...
        var row = '<tr>' +
               ...
               <input id = "grade_public_id" class="form-control" type="hidden" name="grade_public_id[]" ></td>' +
                '<td><input class="form-control" maxlength="4" id="dpt_short_name" name="dpt_short_name[]" type="text"  placeholder="Enter Short Name">\n\
                   </td>' +
    ...
    $(document).on("click", ".add", function () {
        ...
        var input_department_id = $(this).parents("tr").find('#grade_public_id');
         var input_department_name = $(this).parents("tr").find('#classroom_name');
         var input_depatment_short_name = $(this).parents("tr").find('#dpt_short_name');
        ...
        if (!empty) {
            ...
            input_data.each(function () {
                ...
                if (!isDuplicate) {
                   input_department_name.parent("td").html(input_department_name.val() + "<input id='txt_data_name' type='hidden' name='grade_name[]' value='" + input_department_name.val() + "'/><input class='form-control' type='hidden' name='grade_public_id[]' value='" + input_department_id.val() + "'/>");
              input_depatment_short_name.parent("td").html(input_depatment_short_name.val() + "<input id='dpt_short_name' type='hidden' name='dpt_short_name[]' value='"+ input_depatment_short_name.val()+"'/> ");
