 ...
 // Append table with add row form on add new button click
  ...
  var index = $("#edu_example-3 tbody tr:last-child").index();
        var row = '<tr>' +
           ...
           '<td><input class="form-control" type="text" name="class_room_name[]" placeholder="Eenter Class Name"></td>'+ 
 ...
 $(document).on("click", ".add", function(){
  ...
  if(!empty){
   ...
   input.each(function(){
    ...
    if (i==0){
        $(this).parent("td").html($(this).find('input[type="text"] ').text()+"<br><input class='form-control' type='hidden' name='class_room_name[]' placeholder='Eenter Class Name'value='"+$(this).val()+"/>");
